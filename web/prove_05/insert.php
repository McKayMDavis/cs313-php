...
if ($tname == 'expense') {
	for ($i = 0; $i < $nrow; $i++) {
		$db->prepare('INSERT INTO expense (description, vendor, amount, year, date_entered, last_update, goal_id) VALUES (:description, :vendor, :amount, :year, CURRENT_DATE, 1, (SELECT goal_id FROM goal WHERE year=:bob))');
		$db->execute(array(':description' => $data[$i][0], ':vendor' => $data[$i][1], ':amount' => $data[$i][2], ':year' => $data[$i][3], ':bob' => $data[$i][3]));
	}
